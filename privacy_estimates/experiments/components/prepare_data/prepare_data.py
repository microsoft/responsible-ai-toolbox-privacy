...
def _prepare_data(train_base_ds: Dataset, validation_base_ds: Dataset, in_out_ds: Dataset, in_sample_indices_ds: Dataset,
    ...
    Args:
        ...
        merge_unused_samples_with_train_data (bool, optional): Whether to merge unused samples with the train data. Defaults to False.
    """
    ...
    sample_index_to_base_index = {
        (split, sample_index): base_index for base_index, (split, sample_index) in enumerate(zip(in_out_ds["split"], in_out_ds["sample_index"]))
    }
    ...
    assert len(in_sample_indices_ds) == len(out_sample_indices_ds), "in_data_indices and out_data_indices must have the same length"

    if sample_selection == SampleSelectionMethod.INDEPENDENT:
        indices_for_model = np.random.default_rng(seed=seed).choice(np.arange(len(in_sample_indices_ds)), num_points_per_model, replace=True)
    ...
    in_base_indices = [sample_index_to_base_index[(split, sample_index)] for split, sample_index in zip(in_sample_indices["split"], in_sample_indices["sample_index"])]
    out_base_indices = [sample_index_to_base_index[(split, sample_index)] for split, sample_index in zip(out_sample_indices["split"], out_sample_indices["sample_index"])]
    ...
    if (
        ...
        in_sample_indices_not_selected = in_sample_indices_ds.select(np.setdiff1d(np.arange(len(in_sample_indices_ds)), indices_for_model), keep_in_memory=True)
        in_base_indices_not_selected = [sample_index_to_base_index[(split, sample_index)] for split, sample_index in zip(in_sample_indices_not_selected["split"], in_sample_indices_not_selected["sample_index"])]
        in_data_not_selected_ds = in_out_ds.select(in_base_indices_not_selected)

        out_sample_indices_not_selected = out_sample_indices_ds.select(np.setdiff1d(np.arange(len(out_sample_indices_ds)), indices_for_model), keep_in_memory=True)
        out_base_indices_not_selected = [sample_index_to_base_index[(split, sample_index)] for split, sample_index in zip(out_sample_indices_not_selected["split"], out_sample_indices_not_selected["sample_index"])]
    ...
    train_data_for_model_sample_indices = [f"{s}-{i}" for s, i in zip(train_data_for_model_ds["split"], train_data_for_model_ds["sample_index"]) if s is not None and i is not None]
    validation_data_for_model_sample_indices = [f"{s}-{i}" for s, i in zip(validation_data_for_model_ds["split"], validation_data_for_model_ds["sample_index"]) if s is not None and i is not None]
    ...
    out_data_for_model_sample_indices = [f"{s}-{i}" for s, i in zip(out_data_for_model_ds["split"], out_data_for_model_ds["sample_index"]) if s is not None and i is not None]
