...
from privacy_estimates.experiments.components import (
    prepare_data, aggregate_output, filter_aux_data, reinsert_aux_data, append_column_constant
)


class ComponentLoader(ABC):
    def __init__(self, aml_component_loader: AMLComponentLoader) -> None:
...
class TrainSingleModelAndPredictLoader:
    def __init__(self, arguments: TrainSingleModelAndPredictArguments):
        ...
        Args:
            ...
            sample_selection: Method for selecting samples
            tag_model_index: Whether to tag the predictions with the model index which will append a column to the predictions
            merge_unused_samples: Method for merging unused samples with the train/validation data
        """
    ...
    def load(self, train_base_data: Input, validation_base_data, in_out_data: Input, in_indices: Input, out_indices: Input,
        ...
        def p(train_base_data: Input, validation_base_data: Input, in_out_data: Input, in_indices: Input, out_indices: Input,
            ...
            data_for_model = prepare_data(
                ...
                model_index=model_index, sample_selection=self.sample_selection, merge_unused_samples=self.merge_unused_samples
            )
            ...
            inference_in = self.inference_loader.load(
                dataset=filter_in_data.outputs.filtered, model=train.outputs.model
            )
            inference_out = self.inference_loader.load(
