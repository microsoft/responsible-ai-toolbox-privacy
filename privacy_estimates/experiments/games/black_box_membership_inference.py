...
class BlackBoxMembershipInferenceGameBase(ExperimentBase):
    ...
    def pipeline(self, train_data: Input, validation_data: Input) -> PipelineJob:
        ...
        def game_pipeline(train_data: Input, validation_data: Input):
            ...
            if self.attack_loader.requires_shadow_model_statistics:
                attack = self.attack_loader.load(
                    challenge_points=convert_to_challenge.outputs.challenge,
                    shadow_model_statistics=shadow_model_statistics,
                )
            else:
                attack = self.attack_loader.load(challenge_points=convert_to_challenge.outputs.challenge)
