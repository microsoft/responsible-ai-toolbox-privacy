...
def collect_signal_lira(models_path,
    ...
    dataset = np.arange(int(config["data"]["dataset_size"]))

    # dataset_name = get_dataset_name_from_logdir(log_dir)
    ...
    nb_augmentations = int(config["audit"]["nb_augmentation"])
    ...
    def load_or_compute_logits():
        ...
        model_logits = create_or_load_signal_file(signal_path, dataset, num_queries=10, num_augmentations=nb_augmentations) # TODO hard coded number of classes
    ...
    if signal_name == "softmax_relative" :
        ...
        model_signal = create_or_load_signal_file(signal_path, dataset, num_augmentations=nb_augmentations)
...
def aggregate_signals(signal_name, 
    ...
    # pop_reference_signals = reference_signals[:,population_indices] # for the moment we consider what is not targeted as reference shape 1 x 10k
    ...
    if signal_name == "softmax_relative" or signal_name == "taylor_softmax_relative" or signal_name == "sm_taylor_softmax_relative" or signal_name == "sm_softmax_relative": # relative attack
        ...
        if int(configs["audit"]["top_k"]) != -1 : # selects for each x a random set of population equal to int(configs["audit"]["top_k"])
            ...
            values, indices = target_distances.topk(top_k, largest=False, dim=1)
        ...
        if configs["audit"]["augmentation"] == "augmented": # using augmentation
            ...
            if return_proba_ratio:
                return prob_ratio_x, 1/prob_ratio_z_rev
...
if __name__ == "__main__":
    args = Arguments(
        audit_signal="sm_taylor_softmax_relative",
        taylor_m=0.6,
        taylor_n=4
    )
    configs = {
        ...
        "audit": {
            ...
            "signal": args.audit_signal,
            "nb_augmentation": 1,
            "augmentation": "none",
            "taylor_m": args.taylor_m,
            "taylor_n": args.taylor_n,
            "temperature": 1.0
        },
        "data": {
            "dataset_size": 10,
    ...
    if privacy_game == "privacy_loss_model":
        ...
        signals = np.random.rand(nb_models_target, configs["data"]["dataset_size"], 1)
