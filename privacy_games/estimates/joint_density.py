...
from privacy_games.estimates.binomial_proportion import compute_eps_lo_hi
...
class DensityModel(ABC):
    ...
    def eps_lo(self, delta: float, alpha: float=0.05, xtol: float = 1e-2, bracket: Tuple[float, float] = None, max_eps: float = 10.0) -> float:
        ...
        if bracket is None:
            # Narrow down seacrh bracket using an 90% confidence Jeffreys interval
            # The 90% confidence level is arbitrary, but seems safe in practice 
            bracket = list(compute_eps_lo_hi(count=self.count, delta=delta, alpha=min((1-alpha)/2,alpha/2), method="jeffreys"))
